...
;$(function(){
		        ...
		        "button": [
                     {
                         "type": "view",
                         "name": "趣商城",
                         "url": "http://testwebchats.7shengqian.com/index.html",
                         "sub_button": [ ]
                    },
                   {
                       "type": "view",
                       "name": "银嘉",
                       "url": "http://testwebchats.7shengqian.com/?#/travel/index",
                       "sub_button": [ ]
                    },
		            {
                        "name": "市场",
                        "sub_button": [
                            {
                                "type": "view",
                                "name": "云市场",
                                "url": "http://testshop.xinxiangfuwu.com/#/home",
                            }
                        ]
		            }
		        ]
	...
	$('.iBtn').parent().on('click',function(){
        ...
        firstLevelMenu.bind(this)();
    /*

	    console.dir("444444444444");
	    console.dir("ix start:"+ix)
...
*/
	...
	function firstLevelMenu(){
        ...
        if(ix<4){
            ...
            _this.before(customEl);
            _this.prev().append(customUl);

            $('.custom-menu-view__footer__right').find('.subbutton__actived').removeClass('subbutton__actived');
            _this.prev().addClass('subbutton__actived');
            ...
            var divText=_this.prev().find('.text-ellipsis').text();
            button.push(new parents(divText));
        }
        if(ix==3){
            $(this).parents('.custom-menu-view__menu').remove();
            _this.append(customUl);
            $('.custom-menu-view__menu')[ix-1].setAttribute('alt',ix-1);
        }
        $('.cm-edit-after').show().siblings().hide();
    }
	...
	customBtns.on('click','.text-ellipsis',function(){
		...
		if($(this).parent().attr('id')){
            $("#namelimit").html("字数不超过8个汉字或16个字母\n");
	...
	//保存右侧菜单名称
    var flag = true;
    $('input[name="custom_input_title"]').on('compositionstart',function(){
        flag = false;
    })
    $('input[name="custom_input_title"]').on('compositionend',function(){
        flag = true;
    })
    $('input[name="custom_input_title"]').on('input',function(){
        ...
        setTimeout(function(){
            ...
            var nub = 0,testleg = 0,current=$('.subbutton__actived');
            var val = $(this).val();
            var two = $('.custom-menu-view__menu__sub__add').hasClass('subbutton__actived');
            var one = $('.custom-menu-view__menu').hasClass('subbutton__actived');

            if(val){

                val = val.match(/[a-zA-Z\u4e00-\u9fa5]/g).toString().replace(/,/g,'')
            }
            $(this).val(val)
            //二级
            if(two){
                ...
                for(var i =0;i<val.length; i++){
                    ...
                    if (/[\u4e00-\u9fa5]/.test(val[i]) && testleg<16){
                        nub++
                        testleg += 2
                    }
                }
                if(val.length+nub>16){
                    $(this).val(val.substring(0,16-nub))
                }
                var row=current.attr('id').lastIndexOf('_')-1;
                var col=current.attr('id').lastIndexOf('_')+1;
                var sub_row=current.attr('id').substr(row,1);
                var sub_col=current.attr('id').substring(col);
                button[sub_row].sub_button[sub_col].name=$(this).val();
                current.find('.text-ellipsis').text($(this).val());
            }else if(one){
                ...
                if(val.length+nub>8){
                    ...
                    $(this).val(val.substring(0,8-nub))
                }
                var alt=current.attr('alt');
                button[alt].name=$(this).val();
                current.children('.text-ellipsis').text($(this).val());
            }
            updateTit($(this).val())
        }.bind(this),0)
    })
	...
	$('#delMenu').click(function(){
		...
		var two_Actived=$('.custom-menu-view__menu__sub__add').hasClass('subbutton__actived');//二级菜单选中项
		var rowIndex=0;
		var colIndex=0;
		...
		if(is_Actived){
            ...
            if(ix==2){
                ...
                var sortIndex=true;
                customBtns.append(menuI);
                console.log($('.custom-menu-view__menu'))
                $('.cm-edit-before').siblings().hide();
                $('.iBtn').parent().on('click',function(){
	...
	$('#saveBtns').click(function(){
        ...
        var buttongLen = button.length;
        //判断是否有一级菜单
        if(button && buttongLen>0){

            //循环一级菜单 button
            for(i=0;i<buttongLen;i++){
                if(!button[i].name){

                    tipEr.content("菜单名不能为空").show();
                    $('.custom-menu-view__footer__right').find('.custom-menu-view__menu')[i].children[0].click()
                    return false;
                }
                //判断是否有二级菜单
                if(button[i].sub_button.length>0){
                    ...
                    //循环二级菜单
                    for (j=0;j<button[i].sub_button.length;j++){

                        if(!button[i].sub_button[j].name){

                            $("#sub_"+i+"_"+j).find('.text-ellipsis').trigger("click")
                            tipEr.content("菜单名不能为空").show();
                            return false;
                        }
                        var istype = false
                        if(!button[i].sub_button[j]['type']){

                            $("#sub_"+i+"_"+j).find('.text-ellipsis').trigger("click")
                            tipEr.content(button[i].sub_button[j].name+":请完善菜单内容").show();
                            return false
                        }

                        //判断地址是否有误  跳转网页
                        if((!button[i].sub_button[j].url || !re.test(button[i].sub_button[j].url)) && button[i].sub_button[j].type=="view"){

                            $("#sub_"+i+"_"+j).find('.text-ellipsis').trigger("click")
                            tipEr.content(button[i].sub_button[j].name+":url格式有误").show();
                            return false;
                        }
                    }
                }else{
                    //判断地址是否有误  跳转网页
                    if((!button[i].url || !re.test(button[i].url)) && button[i].type=="view"){

                        $('.custom-menu-view__footer__right').find('.custom-menu-view__menu')[i].children[0].click()
                        tipEr.content(button[i].name+":url格式有误").show();
                        return false;
                    }
                }
		...
		$.ajax({
			...
			success : function(re) {
                ...
                if(obj.code == "10000"){
                    ...
                    remarkDialog.title("提示...");
                    remarkDialog.content(obj.info).show();
                    //$.messager.alert("发布成功", re.info, 'info');
				}else{
                    remarkDialog.title("提示...");
                    remarkDialog.content("发布失败："+obj.info).show();
                    //$.messager.alert("发布失败", re.info, 'error');
                }
			}
    	});
	}
