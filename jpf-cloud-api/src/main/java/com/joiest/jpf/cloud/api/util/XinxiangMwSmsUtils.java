...
public class XinxiangMwSmsUtils {
    ...
    */
    ...
    public int sendSms(String strMobiles, String strMessage)
    {
        ...
        sbf.append("\n请求地址：" + ConfigUtil.getValue("XINXIANG_MW_MONGATESENDSUBMITURL"));
    ...
    public int SendMultixSms(List<MwmultSmsInfo> MultixMt)
    {
        ...
        try {
            ...
            Message = OkHttpUtils.executeMwPost(param, ConfigUtil.getValue("XINXIANG_MW_MONGATEMULTIXSENDURL"));//调用底层POST方法提交
            //请求返回值不为空，则解析返回值
            if(Message != null&& Message != "")
            {
                Document doc= DocumentHelper.parseText(Message);
        ...
        SimpleDateFormat myfmt = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        sbf.append("\n\nTime:" + myfmt.format(date));
        sbf.append("\n请求地址：" + ConfigUtil.getValue("XINXIANG_MW_MONGATEMULTIXSENDURL"));
        sbf.append("\n接口参数：" + JSONObject.fromObject(param));
        sbf.append("\n回调信息：" + Message);
        String fileName = "MwSmslog";
