...
public class LogsCustomUtils2 {
    ...
    public static boolean writeIntoFile(String log, String filePath, boolean isAppend) throws Exception{
...
//            fileWriter = new FileWriter(file, isAppend);
    ...
    /** 日志名称：文件名称2018-05.txt
     * 将指定字符串写入文件。如果给定的文件路径不存在，将新建文件后写入。
     * @param log       要写入文件的字符串
     * @param path      文件父级路径的字符串表示形式，目录的层次分隔可以是“/”也可以是“\\”; 不包含文件名称
     * @param fileName      文件名称。
     * @param isAppend      true：追加到文件的末尾；false：以覆盖原文件的方式写入
     */
    public static boolean writeIntoFile(String log, String path, String fileName, boolean isAppend) throws Exception{
        SimpleDateFormat myfmt = new SimpleDateFormat("yyyy-MM");
        String ext = myfmt.format(new Date()) + ".txt";
        boolean isSuccess = true;
        String filePath;
